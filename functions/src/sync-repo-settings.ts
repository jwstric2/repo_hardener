...
import { RepoConfig, BranchProtectionRule, PermissionRule } from './types';
...
export class SyncRepoSettings {
  ...
  async syncRepoSettings(options: SyncRepoSettingsOptions) {
    ...
    const defaultBranch = options.defaultBranch || (await this.getDefaultBranch(repo));
  ...
  async updateBranchProtection(repo: string, rule: BranchProtectionRule) {
    ...
    try {
      ...
      if (err.status === 401) {
        logger.warn(`updateBranchProtection: warning received ${err.status} updating ${owner}/${name}`);
      } else {
  ...
  async updateRepoTeams(repo: string, rules: PermissionRule[]) {
    ...
    try {
      ...
      if (knownErrors.includes(err.status)) {
        logger.warn(`updateRepoTeams: warning received ${err.status} updating ${owner}/${name}`);
      } else {
  ...
  async updateRepoOptions(repo: string, config: RepoConfig) {
    ...
    try {
      ...
      if (knownErrors.includes(err.status)) {
        logger.warn(`updateRepoOptions: warning received ${err.status} updating ${owner}/${name}`);
      } else {
