...
export class Logger {
  ...
  private initPinoLogger() {
    ...
    this.pino = pino(defaultOptions, pino.destination({sync: true}));
  }
...
/**
 * Maps Pino's number-based levels to string-based severity.
 * This allows Pino logs to show up with the correct severity in Logs Viewer.
 * Also preserves the original Pino level
 * @param _label the label used by Pino for the level property
